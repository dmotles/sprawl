...
- **`make build` now relinks to byte-identical binaries on an unchanged tree, and `sprawl version`'s `built:` line reports HEAD's commit date rather than wall-clock build time** (QUM-1287) — for `make` builds. Release binaries are unaffected: `.goreleaser.yaml` stamps the date independently with `{{.Date}}` and keeps wall-clock semantics, deliberately left alone because the template change could not be verified locally. `LDFLAGS` stamped `-X main.date=$(date -u ...)`, a value that changes on every invocation, so the link output could never be reused: every `make validate` — that is, every pre-commit hook, for every agent — relinked both binaries even when nothing had changed. The stamp is now HEAD's committer date (the reproducible-builds `SOURCE_DATE_EPOCH` convention), making the link a function of the tree. Measured: two consecutive builds went from differing hashes to identical ones, and a stamp-only relink costs ~1.3s against ~0.2s for a cached one.

  **`built:` under-reports on a dirty tree** — a binary built from uncommitted work claims its parent commit's time. That is inherent to making the link cacheable, and the value's only surface is `sprawl version`; nothing branches on it. Outside a git checkout the stamp falls back to `unknown`, which is already `internal/buildinfo`'s default for an unstamped build. New gate: `make test-build-stamp` (in `validate`) hashes two consecutive builds, with a sensitivity control that varies the stamp so the equality cannot be vacuous.

  **`fmt-check` was NOT collapsed into `lint`, and that is a finding rather than an omission.** `golangci-lint run` does report formatter findings in v2 — measured, `File is not properly formatted (gofumpt)`, including in `_test.go` files — so the second pass looks redundant. It is not: `run` only loads files that satisfy the host's build constraints, while `fmt` walks the source, and this tree has 17 build-constraint-excluded files in 6 packages. Collapsing them would have silently stopped checking all 17. The premise is now asserted in both halves (`scripts/test-lint-pin.sh` C1 that such files still exist, C2 that `fmt` really inspects them while `run` really does not, with C2c as the control aimed at the opposite verdict), so if golangci-lint ever closes the gap the collapse gets re-evaluated instead of the recorded finding quietly going stale. The full rationale lives in the `fmt-check` comment in the `Makefile`; the copies elsewhere point at it. The lint saving actually taken is a new `FMT_SCOPE` seam, mirroring `LINT_SCOPE`: the pin suite's `fmt-check` legs ask which binary ran, not what it found, so they no longer walk `./...` — `make test-lint-pin` went from 12.8s to ~7.1s, and it narrows the window in which other agents' `validate` hits golangci-lint's machine-wide lock.

- **Every claude session sprawl launches now runs at `--effort low`, with `oracle` deliberately pinned to `medium`** (QUM-1276) — a default change only: no model changed, and there is no config knob or `--effort` flag to opt out of. weave previously set no effort at all (it inherited whatever the `claude` CLI defaulted to) and spawned agents were hardcoded to `medium`; both are now `low`.

  The in-repo sidechain definition `.claude/agents/oracle.md` declares `effort: medium` **explicitly**, because agent frontmatter *inherits* the session effort — leaving it undeclared would have silently dragged the read-only deep-thinking planner down to `low` along with everything else. `.claude/agents/test-critic.md` is deliberately left undeclared: inheritance gives it `low`, which is what it should run at. The built-in sidechains with no in-repo definition (Explore, Plan, general-purpose, claude) likewise inherit, so **they now run low too**.

  **Caveat worth knowing before you edit an agent definition: an invalid or typo'd `effort:` fails silently.** The CLI's frontmatter check is a shadow validator — a bad value or unrecognised key only fires telemetry and writes to the per-session debug log; the file still loads and the agent quietly runs at the inherited effort. So the presence of an `effort:` line is not evidence the agent runs at that level. The value must be one of `low`, `medium`, `high`, `xhigh`, `max`, or an integer.

- **`send_message`'s `interrupt` parameter is renamed `now`, the body is capped at 300 characters, and an unconfirmed delivery is no longer reported as success** (QUM-1186, slice 1 of QUM-1185) — three changes to the one remaining way agents reach each other.

  **Action required — `interrupt` is now a hard error, not an ignored key.** `send_message({to, body, now})`; `now` is a boolean defaulting to false with the same preemption semantics the old parameter had (ancestor-gated, queue-jumping, best-effort mid-turn). Passing `interrupt` is **refused** rather than dropped, whatever its type. That refusal is the point: `encoding/json` silently discards an unknown property, so ignoring the old name would have left a stale caller's urgent message quietly downgraded to cooperative delivery with nobody told — the exact silent-false-claim defect this epic exists to remove, reintroduced by the fix for it. A loud break is the cheap outcome here. **`SendMessageResult`'s JSON field `interrupted` is likewise renamed `now`**, so a shape-dependent parse of the tool's result breaks; the response was otherwise left advertising a parameter that no longer exists. Note the on-disk queue class is still `interrupt` — the name is embedded in queue filenames and renaming it would be a migration for no gain.

  **A `body` over 300 characters is rejected outright — never truncated.** Truncation silently loses content, which is the failure class this epic is about, so the limit is enforced as an error naming the limit, your actual length, and what to do instead. The cap is rune-counted (a 300-character message of non-ASCII is not secretly a 600-byte rejection) and is evaluated before anything else happens, so a rejected send leaves the recipient's inbox and queue untouched and does not wake a completed agent. **Spawn prompts are deliberately uncapped** — the cap exists to push substantive briefs into the project's tracker, where they outlive the message, and a spawn prompt has nowhere else to go.

  **A timed-out stdin injection now reaches the caller, and it says what actually happened.** Previously the bounded write that protects against a wedged recipient (QUM-1072) logged a warning and returned success, so `send_message` reported a delivery that had not been confirmed. It now returns an error — but deliberately **not** "delivery failed": the message is already durable in the recipient's queue before the write is attempted and is retried on the next poke, so a failure claim would replace a silent lie with a loud one. The signal is *delivery not confirmed, the message remains queued*, plus a next action. It is also not attributed to your specific message, because the drain flushes whatever is pending rather than only your entry; the per-entry detail stays in the operator log, where it belongs, instead of leaking other agents' queued mail into your tool result. Paths with no caller to inform — the root's redrain ticker and post-start hook — keep the warning, which is not redundant with the new return value but the only channel they have.

### Fixed

- **The `internal/memory` handoff warn no longer prints a DSN** (QUM-1294) — the fourth unredacted sink in this chain, and the first one inside the `sprawl` binary that no wrap point could reach.

  `recordHandoffInEventLog` logs `store.Process`'s failure through `slog.Default()` and then **swallows** it, which is the right call for a handoff — losing the session summary because the event log is unhappy would trade the artifact a handoff exists to produce for the thing that merely observes it — but it means the print is the only place the error surfaces, and `cmd/root.go`'s `Execute` sink only ever sees errors that are RETURNED. Neither of QUM-1280's wrap points reaches it either: `store.Open`'s `cfg.Logger` is built *after* the failing return, and `cmd/store_dispatch.go`'s handler belongs to a different command. Measured shape: a malformed DSN makes pgx build the error *from* the connection string and quote the whole URL. pgx masks the password; **the user, host and database name leaked**, and in a public repo each is employer-internal detail in its own right.

  The fix wraps the LOGGER with `store.RedactingLogger` at an extracted `warnLedgerUnusable(log *slog.Logger, err error)` seam — a third wrap point of the mechanism QUM-1280 already audited, not a fourth *kind* of fix. Redaction stays at the PRINT, so the error value is untouched and `errors.Is` is unaffected by construction. The sibling `warnHandoffPanic` warn gets the same treatment: the recovered value can be any pgx error from under `store.Process`, and a neighbouring line printing the same class of value unredacted is how the next sink gets born.

  **Two rejected options, recorded because a fourth point-fix needed an argument beyond being small.** Having `store.Process` return a pre-redacted error reaches every caller but redacts at the RETURN, which breaks the `errors.Is` identity `TestStoreDispatch_PropagatesAnOpenFailure` depends on. Wrapping `slog.Default()` process-wide is the only option that pre-empts sink number five — and its cheap form does not work: `slog.SetDefault(slog.New(wrap(slog.Default().Handler())))` **deadlocks the process on its first log line**, because `SetDefault` points the `log` package at the new handler while `slog.Default()`'s handler is the internal `*defaultHandler` that writes back through `log`, so the wrapper re-enters `log.Logger`'s own mutex. Measured on go1.26, not argued. The only working form installs a fresh concrete handler, which changes the output format of every `slog.Default()` site in the binary at once — a repo-wide output decision, not a leak fix. Recorded on **QUM-1296**, which owns the durable class guard, so it is not designed around a construction that cannot work.

  **Coverage, stated as a measured property rather than a completed enumeration** — this is the fifth scope claim in this chain and the previous four were all falsified by a wider grep. What was measured, at this commit: `grep -rn "store\.Process(\|store\.OpenProcessLedger(\|store\.Open(\|store\.NewPGStore(" --include="*.go" cmd/ internal/ | grep -v _test.go` returns three callers outside `internal/store` — this one, `internal/supervisor/ledger_subscriber.go:66` (discards the error without printing; clean), and `cmd/hubd/main.go:61` (**QUM-1292**, still open). That is a statement about what that grep found, not a claim that no fifth sink exists.

  Testing note: `store.Process` is a `sync.Once` over package globals using `slog.Default()`, so the sink cannot be pinned through `Process` — hence the logger seam. The tests drive real driver errors (the parse class through the production `store.OpenProcessLedger`, the connect class through `pgxpool` with an injected `LookupFunc`) rather than hand-written lookalikes, assert the ABSENCE of the specific secret substrings rather than the presence of a `[redacted]` marker, and separately assert the diagnosis survived — an absence-only assertion is satisfied by printing nothing. The connect class is documented as prospective hardening, not live coverage: `store.Open` currently routes connect failures into degraded mode with a nil error, so that shape does not reach this sink today.

- **`make fmt-check` could pass without checking anything** (QUM-1287). The recipe was `test -z "$(golangci-lint fmt --diff ./...)"`, which captured stdout and discarded the exit status — so any formatter failure that printed nothing to stdout was a green. Measured on the parent commit: `make fmt-check GOLANGCI_LINT=false` returned **0**, and a formatter exiting 7 with its diagnostic on stderr also returned **0** while printing `boom`. The repo's formatting gate could silently not run. It now reports a non-empty diff and a non-zero status as two distinct failures, neither of which can be silent, and prints the diff it previously swallowed with the `run 'make fmt'` hint last. Held shut by `scripts/test-lint-pin.sh` F1/F2, both watched red first.

- **Database errors are now redacted at two more surfaces** (QUM-1280) — `cmd/root.go`'s `Execute` sink and the `slog` records `internal/store` emits.

  **Two more, not the last two.** An earlier version of this entry said "the last two surfaces"; that was false, and it is worth recording as a pattern rather than a typo. Two unredacted sinks remained at the time of that entry: `internal/memory/handoff_event.go` logged `store.Process`'s error through `slog.Default()`, which neither wrap point below reaches (**QUM-1294**, now fixed — see the entry above), and `cmd/hubd` is a separate `main` with its own print (**QUM-1292**, still open). Four sinks have now been found one at a time, each by a wider grep than the last, so treat any "that is all of them" claim in this chain as unmeasured — **QUM-1296** exists to make sink number five fail the build instead of needing a QA pass to find it.

  `Execute` is the sink for *every* error any cobra command returns, which made it the widest credential carrier among the paths that RETURN an error (a path that swallows its error and logs it never arrives there — that is the shape of QUM-1294): `store migrate`'s pgx failure is produced from the DSN itself, and `store dispatch`'s degraded refusal wraps a pgx connect error. It now renders through `store.RedactError` in `writeExecError`. **Redaction happens at the PRINT, never at the return** — sanitising where the error is produced would replace the value with a string and break `errors.Is` for in-process callers.

  For the log records the fix is a `slog.Handler` wrapper (`store.RedactingHandler`), not ~17 patched call sites: a per-site fix is one chance to miss per site, plus one for the site added next month. It resolves `LogValuer`s, handles the `KindAny` shape `"error", err` actually produces, redacts attrs preformatted by `logger.With(...)`, recurses into both `WithGroup` and `slog.Group`, and passes a clean record through byte-identically.

  **It is wired in two places, because there are two loggers.** `cmd/store_dispatch.go` wraps the dispatch logger — the `"dispatch pass failed, retrying"` WARN fires on a loop during an outage, making it the highest-volume carrier. But the one that mattered most is built *inside* `store.Open`: the `"event log unreachable — running degraded"` WARN, which fires precisely during the outage this work exists for and was measured printing the full connect error verbatim. No wrapping a caller could do at its own `slog.New` reaches it, so `Open` wraps its own logger.

  Also **reworded the bad-DSN hint**, which taught the expected form with a literal `postgres://user:password@host:5432/dbname` example — routed through the redaction it collapsed to `postgres://[redacted]`, deleting the answer. An error message made useless is one somebody deletes the redaction to fix.

- **`store.RedactSecrets` no longer passes a pgx *connect* error through untouched** (QUM-1281) — it redacted DSN-shaped URLs and the `password`/`pgpassword` keywords only, which measured out to a **no-op on every pgx connect error**. `pgconn.ConnectError` re-renders errors in keyword form regardless of the DSN form it was handed, so a URL-form and a keyword-form DSN produce byte-identical text and the URL pattern never fires; the URL form appears only in the rarer *parse* error. On a real outage — the scenario QUM-1279 exists for — the database **user**, the **database name** and, on a DNS failure, the **hostname** all reached an operator's terminal, each of them employer-internal detail CLAUDE.md forbids in a public repo. (The password does not leak in either shape: pgx omits it from connect errors and masks it as `xxxxxx` in parse errors.)

  The keyword pattern now covers `host`, `hostaddr`, `user`, `dbname` and `database` alongside the two password spellings, and its bare-value class excludes `)` so a parenthesised clause keeps its punctuation. `host=` is **not** in a connect error, though, so two further patterns anchored on format strings in the pinned dependency catch the hostname where it actually appears: `net.DNSError`'s `lookup <host> on <server>` and pgconn's per-dial `<addr> (<host>)`.

  The three tail patterns run **only inside a pgx connect error**, gated on `pgconn.ConnectError`'s own unconditional `` failed to connect to `user= `` prefix. Their grammars are not rare — `lookup <token>` collides with a filename, `<ip>:<port> (<token>)` with any one-word parenthetical — and ungated they ate `failed to lookup migration.sql: no such file` and `dial 127.0.0.1:5432 (timeout)`, where the eaten token *is* the diagnosis. The narrowing is deliberate: a hostname emitted in one of those grammars by something other than pgx is no longer redacted.

  **`port=` is deliberately not redacted**, along with `sslmode`, `connect_timeout` and `application_name`: a port carries no credential, "wrong port" is one of the misconfigurations `store doctor` exists to diagnose, and an error message made useless is one somebody deletes the redaction to fix. The governing rule, now stated in `redact.go`: a value the **operator supplied** is redacted, a value the **failure itself narrated** — the address actually dialled, the resolver actually asked — is preserved. A bare hostname in arbitrary prose, IP addresses in that narration, and unix-socket paths are explicit non-goals, each with its reasoning recorded in the file.

- **The `paste-coalesce` e2e row no longer depends on what the tmux pane's shell did with its command string** (QUM-1277) — the row resolved the sprawl pid by walking `pgrep -P $PANE_PID` for a child named `sprawl`. Whether that child exists is a property of the *pane shell*, not of sprawl: given the single command string `e2e_launch_tui` builds, zsh exec-optimizes it (so the pane pid **is** sprawl and has no such child) while dash and bash fork. tmux's `default-shell` on the development host is zsh, so this mandatory gate for `internal/inputcoalesce/` and `cmd/enter.go` failed before asserting anything about paste coalescing — and would have passed on a forking host, which is how it rotted unnoticed.

  The new shared helper `e2e_resolve_pane_process` in `scripts/lib/e2e-common.sh` walks the pane's subtree self-inclusively and recursively, so it never asks what the shell did. It distinguishes "nothing matched under a live pane" (rc 1) from "the pane pid is unusable" (rc 2); both remain hard failures for the row, never a skip. The search stays confined to the pane's own subtree on purpose — resolving from a global argv or environ scan would share a candidate set with other agents' concurrent runs, and the row goes on to `SIGINT` whatever it resolves.
...
- **`delegate` and `report_status` are deleted; `send_message` is the only way to reach another agent** (QUM-1186, slice 1 of QUM-1185) — sprawl had three overlapping ways to make an agent receive text, and `delegate` and `send_message` shared **zero code**: two persisted data structures, two consumers, two failure modes, for one concept. Worse, `delegate` — the tool used to assign all work — produced no unread badge, no banner and no queue view, so **the operator could not see the thing land**. Calling either tool over MCP now returns an unknown-tool error.
  ...
  **Self-reported work state is deleted; liveness is now derived, never asserted.** Agents no longer claim what they are doing — the runtime observes that a process is alive and in a turn. Every liveness answer sprawl gives traces to a process probe, a projection, or an activity timestamp.
  ...
  A new resting state `idle` marks an agent whose runtime was reclaimed for **inactivity** rather than because its work finished, and it renders distinctly in the TUI (a hollow diamond, not the "dormant" glyph that means finished and not the generic idle dot). It is revivable **on demand** — messaging a reclaimed agent brings it back with its session, worktree and branch intact — and is deliberately **excluded from the boot auto-resume loop**, because relaunching every reclaimed agent at each `sprawl enter` would hand back the exact memory the reclamation freed.
  ...
  **Every agent prompt was rewritten, not search-and-replaced.** The five built prompts described `delegate` and `report_status` at ~15 sites, so until this landed every spawned agent was being told to call tools that return unknown-tool errors. `send_message({to, body, now})` is now presented as the sole coordination tool — an assignment is a message — with the 300-character body cap and the uncapped spawn prompt stated explicitly. The `report_status` habit was deliberately **not** moved onto `send_message`: an agent narrating its state over the messaging channel would recreate the deleted tool in prose and flood its parent's inbox with what the runtime already observes. Prompts now say to record work in **the project's tracker**, and name no tracker — sprawl runs on projects with other trackers or none, and each project's own `CLAUDE.md` names it. Root and manager prompts explain what `idle` means so an operator or parent does not read it as "finished".
...
- **`sprawl store dispatch` printed database errors to stderr without redaction on its two best-effort failure paths** (QUM-1279) — the sweep failure and the startup-reconciliation failure both formatted the raw error, so a pgx parse error quoting the DSN it was handed (host, user and database in URL form) landed on an operator's terminal, from which it travels into an issue or a transcript. This repo is public. Both now go through `store.RedactError`, matching the six sites in `cmd/store.go` that already did, and both write to the injected `deps.Stderr` rather than `os.Stderr` — which is what made the leak untestable and is why nothing caught it: the secret is not in the source, it arrives at runtime from the environment. The regression test asserts the ABSENCE of the specific credential and host substrings rather than the presence of a `[redacted]` marker, and asserts the diagnosis survived, since an error redacted into uselessness is one somebody deletes the redaction to fix.

  **Two unredacted DSN carriers on the same stream are knowingly left in place**, filed as QUM-1280: `cmd/root.go`'s `Execute` sink, which prints every returned error including `store migrate`'s, and the `"error", err` slog attributes in `internal/store` that reach the logger `store dispatch` wires onto stderr — the dispatch-retry WARN among them, which during an outage fires on a loop. Note also that `RedactSecrets` only redacts a DSN in URL or `password=`/`pgpassword=` keyword form: a bare hostname elsewhere in a message passes through, and so do `user=`, `host=` and `database=` in keyword form — which is the shape `redact.go` measured pgx *connect* errors to actually produce. Filed as QUM-1281.
