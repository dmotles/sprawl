...
- **Database errors are now redacted on the last two surfaces that still printed them raw** (QUM-1280) — `cmd/root.go`'s `Execute` sink and the `slog` records `internal/store` emits.
