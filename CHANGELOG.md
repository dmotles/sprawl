...
- **Database errors are now redacted at two more surfaces** (QUM-1280) — `cmd/root.go`'s `Execute` sink and the `slog` records `internal/store` emits.
  ...
  **Two more, not the last two.** An earlier version of this entry said "the last two surfaces"; that was false, and it is worth recording as a pattern rather than a typo. Two unredacted sinks remain: `internal/memory/handoff_event.go` logs `store.Process`'s error through `slog.Default()`, which neither wrap point below reaches (**QUM-1294**), and `cmd/hubd` is a separate `main` with its own print (**QUM-1292**). Four sinks have now been found one at a time, each by a wider grep than the last, so treat any "that is all of them" claim in this chain as unmeasured — **QUM-1296** exists to make sink number five fail the build instead of needing a QA pass to find it.
