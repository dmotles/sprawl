...
- **`make build` now relinks to byte-identical binaries on an unchanged tree, and `sprawl version`'s `built:` line reports HEAD's commit date rather than wall-clock build time** (QUM-1287). `LDFLAGS` stamped `-X main.date=$(date -u ...)`, a value that changes on every invocation, so the link output could never be reused: every `make validate` — that is, every pre-commit hook, for every agent — relinked both binaries even when nothing had changed. The stamp is now HEAD's committer date (the reproducible-builds `SOURCE_DATE_EPOCH` convention), making the link a function of the tree. Measured: two consecutive builds went from differing hashes to identical ones, and a stamp-only relink costs ~1.3s against ~0.2s for a cached one.
