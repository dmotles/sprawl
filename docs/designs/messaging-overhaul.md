> Terminology note (2026-06): pre-rename "sub-agent" in this doc refers to what is now called "sidechain" (Claude `Agent`-tool spawns). See `CLAUDE.md` Terminology section.
...
resume) — and give the harness first-class visibility into sidechain activity
...
A second, orthogonal problem: when `weave` spawns a sidechain, it cannot see
...
Sidechain silence makes the manager agent pattern hard to reason about.
...
- `send_interrupt` — **rare** — only for genuinely urgent parent-side
  corrections;
- `report_status` at each meaningful step (sidechains reporting to
