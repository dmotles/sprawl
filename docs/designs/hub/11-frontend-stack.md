...
- **Recommendation: React 19 + Vite + `@connectrpc/connect-web` + `@connectrpc/connect-query`.**
  ...
  **lowest-risk, lowest-familiarity-cost** path for a Go-first maintainer: the
  ...
  build.
- The app is **small** (a live event-log viewer + an instance switcher + a
  bearer-token login). At this size, framework *runtime* differences barely register;
...
- The **live event stream is NOT consumed via TanStack Query** — it's a bare
  ...
  (`ListInstances`, `Session`, `FetchTranscript`, `SubmitInput`).
...
1. **Consume a Connect/protobuf server-stream** (`SubscribeInstance`) and follow
   the **one rule**: replay from `last_seq` → else full log (`FetchTranscript`
   `from_seq=0`) → then live-tail ([`01` §2](01-architecture.md)). This is an
   **append-only log** feeding a scrollback view. There is **no snapshot tier** —
   the durable seq'd transcript serves full-on-fresh / delta-on-reconnect.
2. **Bearer-token login** — the SPA POSTs the single configured bearer token to
...
### 3.4 Bearer-token login (constraint 2)

Framework-agnostic and trivial — **no OIDC, no library.** The SPA POSTs the single
configured bearer token to the `Login` RPC, which returns an **httpOnly session
cookie**; subsequent calls carry the cookie automatically ([`04`](README.md)).
There is no relying-party flow, no PKCE, no IdP parameter, no client secret in the
SPA. Every candidate integrates this identically. **Not a differentiator.**
...
- **Reconnect (the one rule) is app-level, not transport-level.** When the
  ...
  the `for await` loop ends; we re-open with `from_seq` and replay the delta
  against the full log (no snapshot). This logic is identical across all frameworks — it lives in a plain TS module,
  not in components.
...
│    → gap? → FetchTranscript(from_seq=0) → reset store → resume tail │
└───────────────────────────────────────────────────────────────┘
        │ appends                          ▲ unary calls
        ▼                                  │
┌ event store (Zustand / useSyncExternalStore) ┐   ┌ connect-query (TanStack) ┐
│  append-only ring, bounded high-water         │   │ ListInstances / Session  │
│  selectors: tail window, per-instance cursor  │   │ FetchTranscript / SubmitInput│
└───────────────────────────────────────────────┘   └──────────────────────────┘
        │                                            │
        ▼                                            ▼
┌ React view: virtualized log (@tanstack/react-virtual)          ┐
│  + instance switcher (single pane) + cookie-gated shell        │
└─────────────────────────────────────────────────────────────────┘
```
...
- **`connect-query` handles only unary RPCs** — loading/caching/mutation for the
  switcher, transcript fetch, and input submit.
...
- **Session-cookie lifetime / re-login UX:** how long does the httpOnly session
  cookie live before the SPA must re-POST the bearer token to `Login`, and how
  does the SPA surface an expired cookie mid-stream? (Settle alongside [`04`](README.md).)
