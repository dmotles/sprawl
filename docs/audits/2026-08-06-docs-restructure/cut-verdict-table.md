...
**Everything below this paragraph is generated by `cut-verdict.py`; regenerate it rather than hand-editing it.** The generator re-derives every row from `git show c7093cc:CLAUDE.md`, asserts that its blocks cover all 750 non-blank lines of that original, and exits non-zero if any block is unaccounted for — so the counts below are derived, never typed.
