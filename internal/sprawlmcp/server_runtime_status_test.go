...
// QUM-1186: the QUM-1154 REPORT-age cluster was removed from this file —
// peek and status each had last_report_age tests plus the unparseable,
// zero-timestamp, future-timestamp and no-report degradation cases. All of
// them asserted on last_report_at / last_report_age, which are gone from both
// response shapes along with report_status.
//
// The defect QUM-1154 fixed — "a bare state with no timestamp reads as live
// when it is 15 hours stale" — is NOT reintroduced. It survives on the
// OBSERVED activity axis, which is now the only axis: see
// TestToolStatus_LastActivityAge and
// TestToolPeek_LastActivityAgeFromNewestEntry, both untouched above.
