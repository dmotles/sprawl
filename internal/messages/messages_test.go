...
// TestMessage_TypeRoundTrip pins that Message.Type survives a JSON round-trip
// and that an empty Type is omitted.
//
// QUM-1186: it used TypeStatusChange as its sample value. That constant is
// deleted, but the round-trip is a property of the ENVELOPE, not of any one
// type, so the test is re-hosted onto TypeLivenessCheck (the surviving
// non-empty Type) rather than removed.
func TestMessage_TypeRoundTrip(t *testing.T) {
	...
	if got.Type != TypeLivenessCheck {
		t.Errorf("round-trip Type = %q, want %q", got.Type, TypeLivenessCheck)
	}
...
// QUM-1186: the SendStatusChange / DrainStatusChange / status-filter suite
// was removed here — envelope shape, notifier suppression, summary
// truncation, timestamp defaulting, no-sent-copy, FIFO drain-and-remove,
// regular-message isolation, empty-idempotence, the default-view hiding, and
// the `filter:"status"` mode. All of them tested the status_change envelope
// class, which no longer exists.
//
// The TypeLivenessCheck tests below are NOT part of that class and survive
// untouched: liveness_check envelopes are a separate ephemeral probe and are
// still hidden from every messages_list view.

// TestTypeLivenessCheck_ConstantValue pins the wire constant. Bumping this
// without also updating the TUI parser (NotificationKindLivenessCheck) and
// the inboxprompt heartbeat formatter would silently misroute the envelope.
func TestTypeLivenessCheck_ConstantValue(t *testing.T) {
	if TypeLivenessCheck != "liveness_check" {
		t.Errorf("TypeLivenessCheck = %q, want %q", TypeLivenessCheck, "liveness_check")
	}
}
