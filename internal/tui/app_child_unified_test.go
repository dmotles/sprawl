...
// -- QUM-463 (S4-rewrite): child-panel tool lifecycle -----------------------
//
// Pre-S4 these tests pinned the global spinner counter / spinner.TickMsg
// gate. QUM-674 removed the global spinner subsystem; per-item glyphs in
// ToolCallItem replace the synchronized pulse. The behavioural intent is
// preserved here as: live tool call from a child stream lands in BOTH stores
// (vp.HasPendingToolCall + cl-not-Idle), and the matching tool result clears
// them both.
...
func TestChildStream_FirstToolCall_LandsInBothStores(t *testing.T) {
	...
	if buf := app.agentBuffers["alice"]; buf == nil || buf.cl.Idle() {
		t.Errorf("alice cl must report not-Idle after live ToolCallMsg")
	}
}
...
func TestChildStream_ToolResult_ClearsBothStores(t *testing.T) {
	...
	if buf := app.agentBuffers["alice"]; buf == nil || !buf.cl.Idle() {
		t.Errorf("alice cl must restore Idle after ToolResultMsg")
	}
}
