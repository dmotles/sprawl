...
4. Run `+"`make validate`"+` (or the configured validate command from .sprawl/config.yaml). If validate reports lock contention, retry with backoff — do NOT bypass. This box runs many agents concurrently, so a wall-clock-sensitive failure is more likely contention than a regression — diagnose the failure mechanism before rerunning rather than treating a rerun-until-green as a diagnosis (QUM-1126 draws this flake-vs-wrong-signal distinction).
