...
// QUM-1186: the BuildStatusNotification tests were removed here. They pinned
// the wire format of the status_change notification line, a channel deleted
// with report_status. There is no surviving formatter to re-host them onto.
...
// QUM-1186: the QUM-1064 status_change coalescing suite was removed from here
// to EOF — DrainStatusChangeLines and its last-wins-per-agent coalescing, the
// first-appearance ordering, the corrupt-envelope isolation, and the
// same-second burst case.
//
// Worth recording rather than just deleting: that coalescing was the fix for a
// real production incident (the liveness flood, QUM-1064), and these were good
// tests. They are removed only because the CHANNEL they guarded no longer
// exists — status_change envelopes are never written, so there is nothing left
// to coalesce. If a per-agent ephemeral notification channel is ever
// reintroduced, this suite is the prior art to restore, not to reinvent.
